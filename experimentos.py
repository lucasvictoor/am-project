...
# Features do experimento de mitigação: is_cnab220 excluída intencionalmente:
# com ela presente o problema preditivo fica quase trivial (taxa de fraude ~0.77
# em CNAB=220 vs ~0 no restante), tornando o desbalanceamento de classes irrelevante
# e eliminando a diferença entre métodos de mitigação.
FEATURES_MIT = [f for f in FEATURES if f != 'is_cnab220']
CONT_FEATURES_MIT = [f for f in CONT_FEATURES if f != 'is_cnab220']
...
FEATURE_SETS = {
    ...
    'tipo_temporal_4':  ['pct_saldo_gasto', 'zscore_valor_no_titular',
...
def _metricas(y_test, y_pred, s_test):
    ...
    return {
        ...
        'abs_spd':   abs(spd) if not np.isnan(spd) else np.nan,
        'di':        di,
    }
...
def _job_mitigacao(nome_treino):
    df_tr  = carregar_e_preparar_dados(nome_treino)
    testes = {k: _carregar_teste(k) for k in BASES_MAP if k != nome_treino}
    cont_idx = [i for i, c in enumerate(FEATURES_MIT) if c in CONT_FEATURES_MIT]
    resultados = []

    # ── Scaler ajustado nos dados balanceados (base para lr_bal, lr_rw e PR) ─
    df_bal  = _balancear(df_tr, N_TREINO)
    X_bal   = df_bal[FEATURES_MIT].values.astype(np.float64)
    ...
    X_bal_s = X_bal.copy()
    if cont_idx:
        X_bal_s[:, cont_idx] = scaler_bal.fit_transform(X_bal[:, cont_idx])

    # 1) LR sem mitigação — amostra bruta com scaler próprio (sem balanceamento)
    df_bruto  = df_tr.sample(n=min(len(df_tr), N_TREINO), random_state=SEED)
    X_bruto   = df_bruto[FEATURES_MIT].values.astype(np.float64)
    scaler_bruto = StandardScaler()
    X_bruto_s = X_bruto.copy()
    if cont_idx:
        X_bruto_s[:, cont_idx] = scaler_bruto.fit_transform(X_bruto[:, cont_idx])
    ...
    fs_pr     = FEATURES_MIT + [SENSITIVE]
    cont_pr   = [i for i, c in enumerate(fs_pr) if c in CONT_FEATURES_MIT]
    ...
    for nome_teste, df_te in testes.items():
        ...
        X_te         = df_te[FEATURES_MIT].values.astype(np.float64)
        X_te_bal_s   = X_te.copy()
        if cont_idx:
            X_te_bal_s[:, cont_idx] = scaler_bal.transform(X_te[:, cont_idx])
        X_te_bruto_s = X_te.copy()
        if cont_idx:
            X_te_bruto_s[:, cont_idx] = scaler_bruto.transform(X_te[:, cont_idx])
        ...
        for metodo, y_pred in [
            ('lr_sem_mitigacao',        lr_bruto.predict(X_te_bruto_s)),
            ('lr_balanceado',           lr_bal.predict(X_te_bal_s)),
            ('lr_reponderado',          lr_rw.predict(X_te_bal_s)),
